...
import { sortDueDate, sortCompletedDate } from '@/store/filters.js';
...
describe('Test sorting by completed date functions', () => {
    ...
    it('Sorts a list of tasks by completion date', () => {
        ...
        let completedSort = sortCompletedDate(list);
                
        expect(completedSort[0].id).to.equal(0);
        expect(completedSort[1].id).to.equal(1);
        expect(completedSort[2].id).to.equal(2);
    });
});